...
void get_gap_pos(vector<Rect> &black_keys_boundRect, vector<int> &gap_pos) {
...
int get_median(vector<int> &list) {
	sort(list.begin(), list.end());
	int l = list.size();
	return (l % 2 == 0 ? (list[l/2] + list[l/2 + 1])/2 : list[(l+1)/2]);
}

int get_median_error(vector<int> &list, int list_median) {
	int l = list.size();
	vector<int> list_dev(l);
	for (int i = 0; i < l; i++) {
		list_dev[i] = abs(list[i] - list_median);
	}
	return get_median(list_dev);
}


// add/change improperly detected contours
void modify_contours(int pos, int x, int uy, int ly, vvP &contours, vector<Rect> &boundRect, vector<Moments> &mu, vector<Point2f> &mc, bool add_new) {
	vector<Point> temp;
...
void modify_detected(vvP &contours, vector<Rect> &boundRect, vector<Moments> &mu, vector<Point2f> &mc, int n_white_keys, Mat &bg) {
	...
	if (boundRect[0].x > s.width/100) 
		modify_contours(0, 0, 0, avg_white_lower_y, contours, boundRect, mu, mc, true);
	if (boundRect[boundRect.size() - 1].x < 99*s.width/100)
		modify_contours(contours.size(), s.width - 1, 0, avg_white_lower_y, contours, boundRect, mu, mc, true);
	...
	int u_y_median = get_median(u_y);
	int l_y_median = get_median(l_y);
	int dist_median = get_median(dist); 
	int u_y_error_median = get_median_error(u_y, u_y_median);
	int l_y_error_median = get_median_error(l_y, l_y_median);
	...
	for (int i = 0, n = contours.size(); i < n; i++)
		if (abs(boundRect[i].y - u_y_median) > 10*u_y_error_median || abs(boundRect[i].y + boundRect[i].height - l_y_median) > 10*l_y_error_median)	
			modify_contours(i, mc[i].x, u_y_median, l_y_median, contours, boundRect, mu, mc, false);
	...
	while (i < n_white_keys) {
		if (abs(mc[i + 1].x - mc[i].x) > 3.0*dist_median/2)
			modify_contours(i + 1, mc[i].x + dist_median, u_y_median, l_y_median, contours, boundRect, mu, mc, true);
		i++;
	}
}
...
void sort_contours(vvP &contours, vector<Rect> &boundRect) {
	for (int i = 0, n = contours.size(); i < n; i++) {
		int minIndex = i;
		for (int j = i + 1; j < n; j++) {
...
void get_black_keys(Mat &bg, vvP &black_keys, vector<Rect> &black_keys_boundRect) {
	...
	sort_contours(contours, boundRect);
...
void transfer_points(vector<Point> &dest, vector<Point> &src, float x_threshold, bool left_side) {
...
void get_white_keys(Mat &bg, vvP &black_keys, vector<Rect> &black_keys_boundRect, vvP &white_keys) {
	...
	vector<int> gap_pos;
	get_gap_pos(black_keys_boundRect, gap_pos);
	...
	for (int i = 0, n = white_keys.size(); i < n; i++) {
		if(gap_left) {
			...
			transfer_points(white_keys[i], black_keys_poly[k - 1], mc[i].x, false);
			...
			transfer_points(white_keys[i], black_keys_poly[k], mc[i + 1].x, true);
